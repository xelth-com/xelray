...
pub const EN: &[(&str, &str)] = &[
    ...
    // -- 3D organ view ---------------------------------------------------
    // `xelray.organ.*` is a shared key space: the segmentation document
    // generated by `ml/render3d.py` reads the very same keys off xelth.com,
    // so one server-side table dresses both viewers.
    ("xelray.organ.kidney_left", "Left kidney"),
    ("xelray.organ.kidney_right", "Right kidney"),
    ("xelray.organ.liver", "Liver"),
    ("xelray.organ.spleen", "Spleen"),
    ("xelray.organ.pancreas", "Pancreas"),
    ("xelray.organ.aorta", "Aorta"),
    ("xelray.organ.bones", "Bones"),
    ("xelray.organ.tumor_region", "Tumor region (estimate)"),
    ("xelray.organ.ai_disclaimer", "AI segmentation, not a diagnosis."),
    // A bare unit, appended to a number. Kept translatable because not every
    // language writes it in Latin letters.
    ("xelray.organ.unit_ml", "ml"),
    (
        "xelray.viewer3d.hint",
        "drag to rotate · scroll to zoom · right-drag to pan · \
         click an organ to show or hide it",
    ),
    ("xelray.viewer3d.toggle", "Show or hide this organ"),
    // Honest about what failed and what to do: WebGPU (and the WebGL
    // fallback) can be missing, disabled, or blocked by a driver blocklist,
    // and none of that is something the page can fix for the reader.
    (
        "xelray.viewer3d.no_gpu",
        "This browser could not start 3D graphics, so the model cannot be \
         shown. A recent Chrome, Edge or Firefox on a machine with working \
         graphics drivers should work.",
    ),
    // -- help overlay ----------------------------------------------------
    ...
    ("xelray.help.group.model", "3D model"),
